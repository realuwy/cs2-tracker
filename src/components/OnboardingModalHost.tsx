...
export default function OnboardingModalHost() {
  ...
  useEffect(() => { if (sp.get("onboard") === "1") setOpen(true); }, [sp]);
  useEffect(() => {
    const open = () => setOpen(true), close = () => setOpen(false);
    window.addEventListener("onboard:open", open);
    window.addEventListener("onboard:close", close);
    return () => { window.removeEventListener("onboard:open", open); window.removeEventListener("onboard:close", close); };
  }, []);

  if (!open) return null;
  const close = () => { setOpen(false); const u=new URL(window.location.href); u.searchParams.delete("onboard"); router.replace(u.pathname + u.search); };
  ...
  async function createId() {
    ...
    setUserId(id);
    if (email.trim()) {
      await fetch("/api/register-id", { method: "POST", headers: { "content-type":"application/json" }, body: JSON.stringify({ email, userId: id }) }).catch(() => {});
    }
  ...
  function savePastedId() {
    ...
    setUserId(id); close(); router.push("/dashboard");
  }

  async function recover() {
    setBusy(true); setMsg("");
    await fetch("/api/recover", { method: "POST", headers:{ "content-type":"application/json" }, body: JSON.stringify({ email }) });
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-2xl border border-border bg-surface p-6 text-white shadow-2xl">
        ...
        {ENABLE_RECOVERY && tab==="recover" && (<div className="space-y-3">
          <input className="w-full rounded-lg border border-border bg-surface2/70 px-3 py-2" type="email" placeholder="you@example.com" value={email} onChange={(e)=>setEmail(e.target.value)} disabled={busy}/>
